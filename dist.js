(() => {
  ...
  var webpackChunk_default = (key) => {
    ...
    window[key].pop();
    return [wpRequire.c ?? Object.fromEntries(Object.entries(wpRequire.m).map(([k]) => [
      k,
  ...
  var byKeyword = (strs) => (m) => strs.every((s) => Object.keys(m).some((k) => k.toLowerCase().includes(s.toLowerCase())));
  ...
  var byNestedProps = (props) => (m) => isKeyable(m) && Object.values(m).some((v) => isKeyable(v) && props.some((p) => v?.[p] !== void 0));
